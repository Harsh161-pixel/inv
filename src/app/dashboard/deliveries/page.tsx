  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <h1 className="text-2xl font-semibold text-foreground">Delivery orders (outgoing)</h1>
        <Link
        ...
        <table className="w-full text-sm">
          ...
          <tbody>
            {loading ? (
              ...
              <tr><td colSpan={6} className="p-4 text-muted-foreground">No delivery orders</td></tr>
            ) : (
              docs.map((doc) => (
                <tr key={doc.id} className="border-b border-border hover:bg-muted/30">
                  <td className="p-3">{doc.reference ?? "—"}</td>
                  <td className="p-3">{doc.fromWarehouse?.name ?? "—"}</td>
