...
export default function AdjustmentsPage() {
  ...
  useEffect(() => {
    fetch("/api/documents?type=ADJUSTMENT")
      .then((r) => r.json())
      .then(setDocs)
        ...
        <table className="w-full text-sm">
          <thead>
            <tr className="border-b border-border bg-muted/50">
              <th className="text-left p-3">Reason / Reference</th>
          ...
          <tbody>
            {loading ? (
              ...
              <tr><td colSpan={6} className="p-4 text-muted-foreground">No adjustments</td></tr>
            ) : (
              docs.map((doc) => (
                <tr key={doc.id} className="border-b border-border hover:bg-muted/30">
                  <td className="p-3">{doc.reference ?? "—"}</td>
                  <td className="p-3">{doc.toWarehouse?.name ?? doc.fromWarehouse?.name ?? "—"}</td>
